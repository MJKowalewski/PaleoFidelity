...
4. __FidelityBeta__ function assesses live-dead fidelity across sites (offsets in compositional heterogeneity or gradients across sites). (FORTHCOMING)
...
For a single live-dead analysis, format 'live' and 'dead specimen counts as one-row matrices (e.g., rbind(live), rbind(dead)). The same applies when extracting one sample pair from a larger matrix (e.g., rbind(live[7,]).  
...
The function returns also the adjusted fidelity measures ($xc, $yc) corrected for sampling coverage bias. The bias is based on a resampling model simulating 'Perfect Fidelity' in which departures from perfect fidelity are due solely to sampling a perfectly congruent live-dead system. In addition, mean values for all samples and sample groups are also reported (if factor 'gp' is provided).
