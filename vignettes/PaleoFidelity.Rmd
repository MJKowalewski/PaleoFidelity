...
The package "PaleoFidelity" carries out paleontological fidelity analyses focused on live-dead comparisons. Designed for compositional/enumeration data (e.g., specimen counts of taxa), the package can be used for a single site or sets of sites. A grouping factor can be provided to carry out comparative analyses of sites grouped by habitat, region, etc. An additional grouping factor that splits variables (e.g., species) into subsets can be used to assess how fidelity varies across variable sets (e.g., phyla, functional groups, or taphonomic categories).

Currently, the following functions are provided:

1. __FidelitySummary__ performs data assessment prior to any other analysis.

2. __FidelityEst__ performs compositional fidelity analysis based on correlation/similarity measures.

3. __FidelityDiv__ performs alpha diversity/evenness analysis (live-dead offsets in standardized diversity/evenness).

4. __SJPlot__ plots a fidelity cross-plot of correlation and similarity measures 

5. __AlphaPlot__ plots a fidelity cross-plot of live-dead offsets in alpha diversity and evenness

6. __LDPlot__ plots a comparative live-dead plot for the _n_ most common taxa

## Installing PaleoFidelity Package
The most updated beta version of the package is available at github. To install and upload run the following R script (requires 'devtools'):
...
FidData dataset is a single list with 4 items, but datasets can be stored as separate objects. Live and dead datasets must be formatted as matrices and their dimensions must match. The two factor variables need to be univariate.

For a single sample live-dead analysis, format 'live' and 'dead' specimen counts as one-row matrices. The same applies when subsetting one live-dead sample from a matrix (e.g., rbind(live[7,]) and rbind(dead[7,]).  

## FidelitySummary Function
A function _FidelitySummary_ assesses data for format compliance and numerical adequacy. The function allows to filter rare taxa and small samples. A report summarizing the filtered dataset can be requested. This function is called by other PaleoFidelity functions and the data filters should be set in those functions before the analysis is carried out. When the function is applied in its default form to FidData (but with report=TRUE), a report, notes, and one warning are printed.

```{r fidelity summary function}
FidelitySummary(live = FidData$live, dead = FidData$dead, gp = FidData$habitat, report = TRUE)
```

The report summarizes sample and variable data and alerts the users if any data were removed. Also, in this specific example, the report  confirmed that the grouping factor included 2 levels with more than one sample allowing for by-group analyses and produced a warning about small samples with _n_ < 30.
...
Note that 6 sites were lost but the factor levels still included more than 1 site per level. Also, no taxa were lost (i.e., the removed sites did not contain unique species) so data dimensionality was not reduced. How about n.filters = 100?
...
Given that 12 taxa were lost and only 21 sites were retained, 100 may be an unreasonably high filter for this dataset. Once optimal filters are identified, they can be used in other fidelity functions.

## LDPlot Function
A function _LDPlot_ is  designed to analyze and visualize a single live-dead comparison. The function returns a plot that compares the top _n_ species/variables in terms of rank abundance. Unlike other _PaleoFidelity_ functions, this function accepts vectors. It also provides an option to output a report with summary statistics and expected values under the null model of perfect fidelity.
...
The graph indicates low compositional fidelity: (1) very few top live species are present among dead species and vice versa; (2) some of the top live taxa are also completely missing in dead dataset, and vice versa (taxa marked by asterisks); and (3) Spearman rank correlation is low. This is not surprising given that live data include many species without hard skeleton.

The report=TRUE produces a list with 6 items. The first five items provide a numerical and statistical summary. The expected correlation statistics (expected.coeff) and the statistical significance of observed statistics (p.values) are based on resampling simulation explained below. When p = 0, p values are reported as p < 1 / #iterations.   
...
In this example, the expected distribution of spearman rho values approaches rho = 1, which means that the effect of sampling bias is minimal. Inadequate sampling cannot explain the low values of the observed rho values.
...
_FidelityEst_ function returns also the adjusted fidelity measures ($xc, $yc) with a correction  based on a resampling model simulating 'Perfect Fidelity' (discussed above). In addition, mean values for all samples and sample groups are also reported (if factor 'gp' is provided).
...
To explore visually fidelity estimates produced by _FidelityEst_ use _SJPlot_ function.
...
Tyler, C.T., Kowalewski, M., 2017, Surrogate taxa and fossils as reliable proxies of spatial biodiversity patterns in marine benthic communities. _Proceedings of the Royal Society B_ **284**: 20162839. http://dx.doi.org/10.1098/rspb.2016.2839.
